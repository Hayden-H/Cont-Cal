...
function startGamepadListeners() {
    window.addEventListener('gamepadconnected', function (event) {
        ...
        } else if (gamepad.id.toLowerCase().includes('playstation') || gamepad.id.toLowerCase().includes('dualsense')) {
    ...
    for (let i = 0; i < gamepads.length; i++) {
        ...
        if (gamepad && gamepad.vibrationActuator) {
            ...
            gamepad.vibrationActuator.playEffect("dual-rumble", {
                ...
                console.log(`Vibration started successfully on gamepad at index ${i}.`);
            }).catch(error => {
                console.error(`Error starting vibration on gamepad at index ${i}:`, error);
            });
            gamepadFound = true;
            break; // Stop after finding the first gamepad with a vibration actuator
        }
    }

    if (!gamepadFound) {
        console.log("No gamepads with vibration actuator found.");
    }
}
